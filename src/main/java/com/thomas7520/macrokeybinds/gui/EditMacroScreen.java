...
public class EditMacroScreen extends Screen {
    ...
    public void render(PoseStack p_96089_, int p_96090_, int p_96091_, float p_96092_) {
        renderDirtBackground(p_96089_);
        drawCenteredString(p_96089_, this.font, this.title, this.width / 2, 16, 16777215);


        if(macroTypeSelectId != 0) {
            drawString(p_96089_, font, Component.translatable("text.tooltip.timebox"), guiLeft + 47, guiTop / 2 + 21, Color.WHITE.getRGB());
            drawString(p_96089_, font, Component.translatable("text.tooltip.timeboxsub"), guiLeft + 90, guiTop / 2 + 31, Color.WHITE.getRGB());
        }
        ...
        if(!confirmButton.active && confirmButton.isHoveredOrFocused()) {
            renderTooltip(p_96089_, Minecraft.getInstance().font.split(Component.translatable("text.tooltip.editmacro.forgotvalue").withStyle(ChatFormatting.RED), 150), p_96090_, p_96091_);
        }
        ...
        if(actionTypeSelectId == 1 && macroActionBox.isFocused() && !macroActionBox.getValue().isEmpty() && macroActionBox.getValue().startsWith("/")) {
            ...
            drawString(p_96089_, font, Component.translatable("text.tooltip.actionbox"), guiLeft - 45, guiTop / 2 + 85, Color.WHITE.getRGB());
            ...
            drawString(p_96089_, font, Component.translatable("text.tooltip.namebox"), guiLeft - 140, guiTop / 2 - 15, Color.WHITE.getRGB());
            drawString(p_96089_, font, Component.translatable("text.tooltip.actionbox"), guiLeft - 142, guiTop / 2 + 65, Color.WHITE.getRGB());
